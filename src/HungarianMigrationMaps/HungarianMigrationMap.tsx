...
export default function HungarianMigrationMap({ data }: Props) {
  ...
  const startAnimation = () => {
    ...
    data.forEach((point, i) => {
      setTimeout(() => {
        // Show point
        svg.select(`#point-${i}`).transition().duration(300).attr("r", 6);
        ...
        d3.select("#migration-info").style("opacity", 1);
      }, i * stepDuration);
    });
  };
  ...
  useEffect(() => {
    ...
    const projectedPoints = data.map(
      (p) => projection([p.lon, p.lat]) as [number, number],
    );
